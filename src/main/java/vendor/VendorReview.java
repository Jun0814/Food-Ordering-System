...
public class VendorReview extends javax.swing.JPanel {
    ...
    private void intiMenuPanel(String foodCategory){
        ...
        String [][] reviewData = data.retrieveDataAsArray(7, userId, "src\\main\\java\\repository\\food.txt");
        for(String[] data : reviewData){
            try{
                String vendorReferenceId = data.length > 1 ? data[1].trim() : "";
                if (vendorReferenceId.equalsIgnoreCase(userId)) {
                    String reviewId = data.length > 0 ? data[0].trim() : "";
                    String rating = data.length > 2 ? data[2].trim() : "";
                    String reviewComment = data.length > 3 ? data[3].trim() : "";
                }
            }catch(Exception e){
                e.printStackTrace();
            }
        ...
        for (String[] data : foodData) {
            try {
                ...
                if (foodCategory.equalsIgnoreCase(foodType)) {
                    ...
                    foodPanel.setButtonText("Review");
